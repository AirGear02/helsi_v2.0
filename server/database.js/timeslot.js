...
const getTimeslotByDoctorIdAndDate= async (doctorId,dateVisiting)=>{//date from to 7 days
  const doctors=  await sequelize.query(` SELECT "timeSlot"."start_time" AS "start_time", 
...
const getTimeslotByDoctorId= async (doctorId)=>{
  ...
  const getTimeslotByDoctorIdAndHospitalId= async (doctorId,hospitalId)=>{//date from to 7 days
    const doctors=  await sequelize.query(` SELECT "timeSlot"."start_time" AS "start_time", 
      "timeSlot"."end_time" AS "end_time", "timeSlot"."date_visiting" AS "date_visiting",
      "workPlace"."doctor_id" AS "doctor_id"
       FROM "time_slots" AS "timeSlot" INNER JOIN "schedules" AS "schedule"
       ON "timeSlot"."schedule_id" = "schedule"."id" INNER JOIN 
       "work_places" AS "workPlace" ON "schedule"."work_place_id" = "workPlace"."id" 
      WHERE "workPlace"."doctor_id"=:doctor_id AND "workPlace"."hospital_id"=:hospital_id`,
      {
        replacements: { doctor_id: doctorId,hospital_id:hospitalId},
        type: Sequelize.SELECT 
      });
     return doctors[0];
  }
  const getTimeslotByDocIdAndHospIdAndDate= async (doctorId,hospitalId,dateVisiting)=>{//date from to 7 days
    const doctors=  await sequelize.query(` SELECT "timeSlot"."start_time" AS "start_time", 
      "timeSlot"."end_time" AS "end_time", "timeSlot"."date_visiting" AS "date_visiting",
      "workPlace"."doctor_id" AS "doctor_id"
       FROM "time_slots" AS "timeSlot" INNER JOIN "schedules" AS "schedule"
       ...
       "work_places" AS "workPlace" ON "schedule"."work_place_id" = "workPlace"."id" 
      WHERE "workPlace"."doctor_id"=:doctor_id AND "workPlace"."hospital_id"=:hospital_id
      AND "timeSlot"."date_visiting"=:date_visiting`,
      {
        replacements: { doctor_id: doctorId,hospital_id:hospitalId,date_visiting:dateVisiting},
        type: Sequelize.SELECT 
      });
     return doctors[0];
  }
...
module.exports.getTimeslotByDoctorIdAndHospitalId = getTimeslotByDoctorIdAndHospitalId;
module.exports.getTimeslotByDocIdAndHospIdAndDate=getTimeslotByDocIdAndHospIdAndDate;
