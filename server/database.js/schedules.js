...
async function getScheduleById(schedule_id){   
    schedules=await sequelize.query(`Select * From "schedules" where "id"=:schedule_id`,
        {
...
async function getScheduleFree(schedule_id,date_visiting,start_time,end_time){  
   ...
   // console.log(date_visiting);
    
        schedule=await sequelize.query(`Select * From "time_slots" where "schedule_id"=:scheduleId
